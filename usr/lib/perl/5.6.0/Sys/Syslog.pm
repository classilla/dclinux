...
C<_PATH_LOG>.  A value of 'inet' will connect to an INET socket returned
by getservbyname().  Any other value croaks.
...
sub connect {
    ...
    unless ( $sock_type ) {
        ...
        connect(SYSLOG,$that)                            || croak "connect: $!";
						            ...
						            # note that the following won't get called since XS code
						            # croaks if it can't get _PATH_LOG
        my $syslog = &_PATH_LOG()                        || croak "_PATH_LOG not found using h2xs";
