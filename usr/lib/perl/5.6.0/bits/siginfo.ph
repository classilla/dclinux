...
if(!defined (defined(&_SIGNAL_H) ? &_SIGNAL_H : 0)  && !defined (defined(&__need_siginfo_t) ? &__need_siginfo_t : 0)) {
    die("Never include this file directly.  Use <signal.h> instead");
}
require 'bits/wordsize.ph';
if((!defined (defined(&__have_siginfo_t) ? &__have_siginfo_t : 0)  && (defined (defined(&_SIGNAL_H) ? &_SIGNAL_H : 0) || defined (defined(&__need_siginfo_t) ? &__need_siginfo_t : 0)))) {
    eval 'sub __have_siginfo_t () {1;}' unless defined(&__have_siginfo_t);
    eval 'sub __SI_MAX_SIZE () {128;}' unless defined(&__SI_MAX_SIZE);
    if((defined(&__WORDSIZE) ? &__WORDSIZE : 0) == 64) {
...
if(defined (defined(&_SIGNAL_H) ? &_SIGNAL_H : 0)  && !defined (defined(&__have_sigevent_t) ? &__have_sigevent_t : 0)) {
